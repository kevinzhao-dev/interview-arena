...
- The resolved private `progress_dir`: interview history and performance tracker.

Treat `cpp/` as a leetgo scratch workspace. Do not use it as the source of truth. Never commit generated `question.md` files unless the user explicitly asks, because they may contain full LeetCode problem text.
...
The script resolves the repository root from its own module location. Pass an
explicit `root` only when intentionally writing to a different workspace.
...
- recent notes from the `progress_dir` returned by `node scripts/arena-context.mjs`
