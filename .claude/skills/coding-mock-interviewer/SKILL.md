...
description: Use when conducting a coding mock interview, coding teaching session, or focused coding drill for Kai-Wen Zhao's Google L5 / Meta E5 preparation. The interview language is Modern C++; sessions should follow senior SWE interview structure, produce mode-aware feedback, and save progress notes for full mocks.
---
...
7. **Debrief phase**
   ...
   - For Teaching Mode, create a progress note only when Kai-Wen explicitly asks to record it, retest it, add it to the tracker, or agrees during the session that it should be saved.
...
In Teaching Mode, treat the seven items as learning checkpoints. A taught problem is review material, not mastered material, until Kai-Wen can answer the seven items with limited or no prompting in a later Guided Mock or Drill redo.
